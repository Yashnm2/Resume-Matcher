...
7. Prioritize explicit must-have requirements, then preferred qualifications, then responsibility-linked terms; ignore benefits and company boilerplate
8. Scan EVERY section by DEFAULT, but change only relevant evidence. Never force a keyword into an unrelated bullet
9. Put keywords in context with the action, work, and supported outcome; do not create a stuffed keyword list or repeat a term mechanically
10. Preserve every original skill and list item when reordering; place the most job-relevant verified items first
11. Prefer the exact JD phrase and its acronym or full-term variant only when natural and supported by the master resume
...
- The summary is concise, evidence-backed, and front-loads the strongest job match without first-person pronouns
- Each bullet communicates one accomplishment with active voice and a precise verb
- Metrics appear only when supported; otherwise use truthful scope, method, audience, complexity, or qualitative outcome
- Keywords are natural, relevant, and contextual rather than stuffed or repeated
- Existing entries remain in reverse chronology, and every original skill/certification/language/award remains present
