...
- Separate explicit must-have requirements from preferred/nice-to-have
  requirements. Do not promote preferred qualifications into required_skills.
- Order every list by hiring importance: explicit requirements first, then
  repeated or responsibility-linked terms, then secondary context.
- Capture exact multi-word phrases, industry terms, tool names, credentials, and
  both the full term and acronym when the posting supplies both.
- Turn responsibilities into concise capability phrases that can be matched to
  evidence, not vague single words.
- Exclude benefits, equal-opportunity language, company boilerplate, generic
  culture adjectives, and application instructions from keywords.
- Do not infer a requirement merely because it is common for the occupation.
- Infer numeric experience and seniority only when the posting provides enough
  evidence.
...
10. Treat the original resume as the evidence boundary. A plausible claim is
    still unsupported unless the original contains evidence for it.
11. Never manufacture a metric from qualitative wording. If no number exists,
    improve specificity with truthful scope, method, audience, or outcome instead.
12. Every edited claim must be defensible in an interview from the candidate's
    source material.
...
- Prioritize explicit must-haves, then preferred qualifications; do not spend
  edits on low-value boilerplate keywords
- Put the strongest verified match early in the summary and within each relevant
  section, without changing chronology or bullet count
- Write concise action + work + result bullets when the source supports all
  three; otherwise use action + scope or method without inventing a result
...
- Rank edits by requirement importance: explicit must-haves first, preferred
  qualifications second, generic wording last
- Put the strongest verified match early in the summary and order bullets within
  an entry by relevance and impact when reordering is allowed
- Use the job posting's exact phrase or acronym only when it truthfully describes
  the candidate's evidence
- Prefer action + work + result bullets. If the source lacks a result, use
  truthful scope, method, complexity, or audience instead of fabricating impact
- Keep bullets concise, specific, scannable, and free of first-person pronouns,
  keyword stuffing, and generic soft-skill claims
- Do NOT use em dash ("—") anywhere in the writing/output, even if it exists, remove it
...
- Build an implicit requirement-to-evidence map before editing: prioritize
  explicit must-haves, then preferred qualifications, and skip requirements with
  no resume evidence
- Make the top third communicate fit quickly: a concise evidence-backed summary
  followed by the most relevant verified skills and accomplishments
- For bullets, prefer action + project or task + result. Use existing metrics
  where available; otherwise add only supported scope, method, audience, or outcome
- Reorder existing bullets or skills by relevance and impact, but preserve
  reverse chronology and never hide a role or alter dates
- Use exact job terminology naturally and in context; do not stuff keywords,
  repeat them mechanically, or turn soft skills into unsupported self-ratings
- Keep each bullet to one focused accomplishment, normally 1-2 lines, with active
  voice and no first-person pronouns
- Do NOT use em dash ("—") anywhere in the writing/output, even if it exists, remove it
...
- Opening: Name the specific role and organization when available, then give a
  concrete, posting-grounded reason for interest. Do not invent company research
  or use generic excitement.
- Middle: Select the TWO highest-priority job needs that have strong resume
  evidence. For each, connect a specific candidate action or example to the
  employer's need and explain the likely contribution.
- Use the job's language/terminology where the candidate's proven experience supports it
  (e.g., if the resume shows "built automated data pipelines" and the
  job says "ETL," describe that real work as ETL).
- Do not merely restate resume bullets or list skills. Add interpretation: why
  that evidence matters for this role.
- Include metrics only when present in the resume. If no metric exists, use
  truthful scope, method, complexity, or outcome.
- Closing: Briefly restate the contribution and invite a conversation; confident
  and courteous, never desperate.
- If resume shows career transition, frame the pivot as intentional and relevant
- Extract company and role from the job description. If either is absent, write
  naturally without placeholders or guessing.
- Do NOT invent information not in the resume
- Do not invent a hiring manager, referral, company initiative, product, value,
  or personal motivation that is not in the provided inputs.
- Tone: Specific, concise, active, and human. Preserve the candidate's likely
  voice; avoid clichés, flowery praise, generic soft-skill claims, unsupported
  superlatives, and repeated sentence openings with "I".
- Do NOT use em dash ("—") anywhere in the writing/output, even if it exists, remove it
...
- Open with the genuine connection point available in the inputs: a shared
  context, the specific role, team, product, or problem. Never fabricate
  familiarity or a referral.
- Give one strong, evidence-backed reason the candidate may be relevant, using a
  concrete metric only when it exists in the resume.
- Use the posting's terminology naturally only where the resume substantiates it.
- Make the message about learning or a useful conversation, not demanding a job
  or referral.
- End with one clear, low-friction ask, such as a 15-20 minute conversation or
  permission to send a concise question.
- Tone: Warm, direct, respectful of the recipient's time, and natural for a
  professional peer.
- Do NOT include placeholder brackets
- Do NOT use phrases like "excited about", "passionate about", "pick your brain",
  or generic flattery.
- Do NOT restate multiple resume bullets, over-explain the candidate's
  background, or attach unsupported claims to the recipient or company.
- Do NOT use em dash ("—") anywhere in the writing/output, even if it exists, remove it
...
- Rank role-fit observations by explicit must-have requirements, then preferred
  qualifications.
- For each suggested answer, use a compact STAR structure where evidence permits:
  situation or task, the candidate's individual action, and the result. Do not
  invent missing STAR elements; instead tell the candidate what fact to recall.
- Questions should test the strongest claimed matches, ambiguous claims, likely
  technical depth, tradeoffs, ownership, and the highest-priority gaps.
- Include the strongest supported fit and the most consequential evidence gap,
  not just a list of every keyword.
- Talking points must connect one verified resume fact to one job need; avoid
  generic strengths such as "hard-working" or "team player" without evidence.
- Use the employer's terminology only when the resume supports the connection.
...
2. Rank explicit must-haves first, then preferred qualifications, then
   responsibility-linked terms.
3. Include existing resume skills that are highly relevant to the JD.
4. You may include a JD skill missing from the skills list only when another part
   of the resume contains substantive evidence for it; explain that evidence in
   the reason.
5. Do not include skills unrelated to the JD or terms drawn only from benefits or
   company boilerplate.
...
13. Before proposing changes, build a requirement-to-evidence map internally.
    Prioritize explicit must-haves, then preferred qualifications.
    Skip any requirement without supporting resume evidence.
14. Make the top third scannable: the summary should state the truthful target
    fit, strongest relevant capabilities, and best differentiator in 2-4 concise
    lines, without an objective statement or first-person pronouns.
15. Improve bullets toward Action + Project/Task + Result. Use an existing metric
    when available; otherwise use only supported scope, method, complexity,
    audience, or qualitative outcome.
16. Each bullet should communicate one accomplishment, stay concise (normally
    1-2 lines), use active voice, and avoid "responsible for", "helped with",
    "worked on", generic self-ratings, and filler.
17. Use exact JD phrases and acronym/full-term variants only where natural and
    evidence-backed. Never keyword-stuff or force a keyword into an unrelated bullet.
18. Prefer high-signal changes: strongest verified match in the summary, then
    relevant experience or project evidence, then skills ordering. Do not edit
    for synonym variety alone.
19. When reordering a list, preserve every original item. Within a job or project,
    place the most relevant and impactful bullets first; preserve reverse
    chronology across entries.
20. Prefer standard, widely understood terminology. Preserve necessary technical
    specificity, but make the candidate's contribution understandable to a
    recruiter who may not share the candidate's specialist background.
