...
2. Missing outcome or purpose: It names a task but not what changed, improved,
   shipped, enabled, or was learned
...
7. Low signal: It uses generic soft-skill claims or filler instead of observable
   evidence
...
7. Prefer questions that recover the missing part of Action + Project/Task +
   Result, but do not pressure the candidate to invent a metric
8. Ask for truthful alternatives to metrics: scale, method, complexity, audience,
   frequency, constraints, deliverable, or qualitative outcome
...
- Do not mark a strong qualitative accomplishment weak merely because it lacks a
  number
...
- Examples in questions and placeholders are prompts only. Never copy an example
  metric, tool, scope, or result into resume content unless the user confirms it.
...
1. Action-oriented: Start with a precise verb that matches what the candidate
   actually did; do not inflate ownership with "Led" or "Architected"
...
6. Structured around one accomplishment: Action + Project/Task + Result when
   supported, or Action + truthful scope/method when no result was provided
...
IMPORTANT RULES:
- Generate only 1-3 distinct NEW bullet points to ADD (not replace); fewer is
  better when the evidence supports fewer
...
- Never upgrade participation into ownership (for example, "helped" into "led")
...
- Prefer a verified qualitative outcome, purpose, deliverable, or scale when no
  numeric result exists
...
6. Use one focused accomplishment per bullet, ideally Action + Project/Task +
   Result when the source supports it
...
- Never upgrade the candidate's ownership, seniority, or contribution beyond the
  supplied text
- Keep bullets concise (1-2 lines each)
- Prefer precise, common verbs over inflated or AI-sounding verbs
- Do not turn soft skills into unsupported self-ratings; demonstrate them through
  the work described
